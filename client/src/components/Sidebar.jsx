...
const filteredUsers = users
  .filter(user => user._id !== authUser?._id) // exclude logged-in user
  .filter(user => input ? user.fullName.toLowerCase().includes(input.toLowerCase()) : true);
    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
     <div className='pb-5'>
      ...
      <div className='bg-[#282142] rounded-full flex items-center gap-2 '>
        ...
        <img src={assets.search_icon} alt="Search" className='w-3 mx-3'/>
        <input
        onChange = {(e)=>setInput(e.target.value)}
        type="text" className='bg-transparent border-none outline-none m-2 mx-0 text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search User...' />
